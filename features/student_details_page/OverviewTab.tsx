...
export default function OverviewTab({ student }: { student: any }) {
  ...
  const [editModalConfig, setEditModalConfig] = useState<{
    isOpen: boolean;
    tab: "personal" | "guardian" | "medical" | "academic";
  }>({ isOpen: false, tab: "personal" });
  ...
  return (
    <>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <p className="text-2xl font-bold text-green-600">
                ${student.performance?.amountPaid || 0}
              </p>
            </div>
            <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() =>
                  setEditModalConfig({ isOpen: true, tab: "personal" })
                }
                className="text-sm text-teal-600 hover:text-teal-700 font-medium cursor-pointer flex items-center gap-1 bg-transparent border-none"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm font-medium text-gray-900 mt-1">
                  {student.details?.dob || "N/A"}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Admission Date</p>
                <p className="text-sm font-medium text-gray-900 mt-1">
                  {student.details?.admissionDate || "N/A"}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Previous School</p>
                <p className="text-sm font-medium text-gray-900 mt-1">
                  {student.details?.previousSchool || "N/A"}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Home Address</p>
                <p className="text-sm font-medium text-gray-900 mt-1">
                  {student.details?.address || "N/A"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() =>
                  setEditModalConfig({ isOpen: true, tab: "guardian" })
                }
                className="text-sm text-teal-600 hover:text-teal-700 font-medium cursor-pointer flex items-center gap-1 bg-transparent border-none"
              >
                <Edit2 className="w-4 h-4" /> Edit
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-900 mt-1">
                  {student.details?.guardianEmail || "N/A"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              <button
                className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer whitespace-nowrap"
      ...
      {editModalConfig.isOpen && (
        <EditStudentModal
          student={student}
          initialTab={editModalConfig.tab}
